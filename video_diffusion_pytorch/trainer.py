...
class CrossAttentionUNet(nn.Module):
    def __init__(
        ...
        # Positional embeddings and initial configurations
        rotary_emb = RotaryEmbedding(min(32, attn_dim_head))
        ...
        # Temporal attention and layers setup
        self.init_temporal_attn = Residual(PreNorm(init_dim, self.temporal_attn(init_dim)))
        self.downs = nn.ModuleList([])
        self.ups = nn.ModuleList([])
        ...
        cond_dim = cond_dim if use_bert_text_cond else None
        cond_dim = time_dim + int(cond_dim or 0)
        
        # Construct layers for downsampling, middle, and upsampling
        dims = [init_dim, *map(lambda m: dim * m, dim_mults)]
        in_out = list(zip(dims[:-1], dims[1:]))
        num_resolutions = len(in_out)
        block_klass = ResnetBlock
        ...
        for ind, (dim_in, dim_out) in enumerate(in_out):
            ...
            self.downs.append(nn.ModuleList([
                ...
                Residual(PreNorm(dim_out, self.temporal_attn(dim_out))),
                Downsample(dim_out) if not is_last else nn.Identity()
            ]))
        ...
        self.mid_temporal_attn = Residual(PreNorm(mid_dim, self.temporal_attn(mid_dim)))
        self.mid_block2 = block_klass_cond(mid_dim, mid_dim)
        ...
        for ind, (dim_in, dim_out) in enumerate(reversed(in_out)):
            ...
            self.ups.append(nn.ModuleList([
                ...
                Residual(PreNorm(dim_in, self.temporal_attn(dim_in))),
                Upsample(dim_in) if not is_last else nn.Identity()
            ]))

        # Final convolution
        self.final_conv = nn.Sequential(block_klass(dim * 2, dim), nn.Conv3d(dim, out_dim or channels, 1))

    def temporal_attn(self, dim):
        return EinopsToAndFrom("b c f h w", "b (h w) f c", Attention(dim, heads=self.attn_heads, dim_head=self.attn_dim_head, rotary_emb=RotaryEmbedding(dim)))

    def forward(self, x, time, cond=None):
        assert not (self.has_cond and cond is None), "cond must be passed if cond_dim specified"
        ...
        if self.has_cond:
            batch, device = x.shape[0], x.device
            mask = prob_mask_like((batch,), 0., device=device)
            cond = torch.where(rearrange(mask, "b -> b 1"), self.null_cond_emb, cond)
            t = torch.cat((t, cond), dim=-1)
...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, context):
        B, N, C = x.shape
        qkv = self.qkv(torch.cat([x, context], dim=1)).reshape(B, N + context.shape[1], 3, self.num_heads, self.head_dim).permute(2, 0, 3, 1, 4)
...
class CrossAttentionUNet(nn.Module):
    def __init__(
        ...
        self.init_conv = nn.Conv3d(channels, init_dim, kernel_size=init_kernel_size, padding=init_kernel_size // 2)
        ...
        for mult in dim_mults:
            output_dim = dim * mult
            self.encoder_blocks.append(self.conv_block(input_dim, output_dim, block_type))
            self.attn_blocks.append(CrossAttention(output_dim, attn_heads))
            input_dim = output_dim

        # Bottleneck
        self.bottleneck = self.conv_block(input_dim, input_dim * 2, block_type)
        self.cross_attention = CrossAttention(input_dim * 2, attn_heads)
        ...
        for mult in reversed(dim_mults):
            output_dim = dim * mult
            self.upconvs.append(nn.ConvTranspose3d(input_dim * 2, output_dim, kernel_size=2, stride=2))
            self.decoder_blocks.append(self.conv_block(input_dim, output_dim, block_type))
            input_dim = output_dim

        # Final convolution
        self.final_conv = nn.Conv3d(input_dim, out_dim or channels, kernel_size=1)

    def conv_block(self, in_channels, out_channels, block_type):
    ...
    def forward(self, x, context):
        ...
        # Encoder
        encodings = []
        for enc_block, attn_block in zip(self.encoder_blocks, self.attn_blocks):
            x = enc_block(x)
            encodings.append(x)
        ...
        # Decoder
        for upconv, dec_block, enc in zip(self.upconvs, self.decoder_blocks, reversed(encodings)):
            x = upconv(x)
...
class TrainerDDP(object):
    ...
    def save(self, milestone, rank=0):
        data = {
            'step': self.step,
        ...
        if rank == 0:
            ...
            torch.save(data, str(self.results_folder / f'model-{milestone}_{rank}.pt'))
